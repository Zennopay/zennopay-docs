...
- Reorganized guides around QR Payments and Platform Payouts, with a shared
  sidebar and clearer quickstart links.
- Added the [payout funding guide](/platform-payouts/funding) and beneficiary
  listing reference.
- Updated PaymentSheet examples for the v0.7.0 SDKs, including session-token
  argument names and pending outcomes.
- Clarified webhook verification and payment reconciliation guidance.
- Refreshed the layout with Zennopay's typography, product illustrations,
  buttons, and liquid-glass surfaces.
- Retired the combined product explainer; its URL redirects to the welcome page.
...
- [Funds flow](/concepts/funds-flow) now includes a pre-fund → FBO →
  drawdown → local-rail merchant-delivery diagram.
