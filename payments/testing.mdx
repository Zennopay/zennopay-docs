...
<Steps>
  <Step title="Sandbox end-to-end">
    ...
    real VietQR payload (paste-QR works fine). Delivery runs on the sandbox
    rail and the intent reaches `captured`. Assert your `completed` handling
    refreshes wallet state from your backend and renders the receipt, and that your
  ...
  <Step title="Pending and reconciliation">
    If status polling times out or the user leaves during processing, current
    SDKs return `pending` on every platform. The payment may still settle. Verify you reconcile via the webhook or
