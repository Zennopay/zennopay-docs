...
(`in.zennopay:sdk`, on Maven Central). Declare `mavenCentral()` alongside
`google()` in `android/settings.gradle`. The `CAMERA` permission is merged from the native SDK's
...
function PayButton() {
  ...
  const onScanAndPay = async () => {
    ...
    // 2. Present the native PaymentSheet and await the checkout result.
    ...
    // 3. Handle every checkout result.
    switch (result.status) {
      case 'completed':
        showReceipt(result.receipt); // refresh wallet state from your backend
        break;
      case 'pending':
...
const result = await presentSheet({ intentId, sessionJwt: sessionToken, config: { environment: 'sandbox' } });
```
...
`pending` means the user left during processing or status polling timed out — the payment may still settle;
