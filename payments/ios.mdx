        ...
        Task { @MainActor in
            ...
            Zennopay.presentCheckout(
                ...
                case .completed(let intentID):
                    // Refresh wallet state from your backend and show the receipt.
                    self.showReceipt(intentID: intentID)
                case .pending(let intentID):
                    self.showPending(intentID: intentID)
                case .canceled:
...
public enum PaymentResult: Equatable, Sendable {
    /// Wallet debited, merchant delivery captured.
    ...
    /// Confirmed, with delivery still unresolved when the sheet closed.
    case pending(intentID: String)
    /// User dismissed before confirming.
...
`networkError`, …). A `.pending` result means the user left during processing or polling timed
out. The payment may still settle. Show a soft state and reconcile via your
...
1. **Sandbox end-to-end:** run against `.sandbox` with a session from your
   sandbox backend and a real VietQR payload; delivery runs on the sandbox
