...
There is nothing to sign but the API call itself: you **create a payment intent**
server-to-server, and Zennopay **mints the session token** and returns it in the
same response. No RS256 key pair, no `iss`/`kid`, no JWKS — one credential, the
HMAC secret.
...
| Value | Lives | Used for |
|---|---|---|
| HMAC key ID | Server only | Selects the signing secret |
| HMAC signing secret | Server only — never in a client | Signs server-to-server calls |
| App package or bundle identifier | App configuration and Zennopay allowlist | Identifies the approved mobile app |

<Note>
  **No signing key pair.** There is no partner-held RSA keypair and nothing to
  publish at a JWKS endpoint — Zennopay mints the session token.
</Note>
...
An Express integration example using `express` and Node's `node:crypto`.
The application helpers below are placeholders for your authentication,
wallet authorization, durable checkout store, and intent-ownership checks.
Use the [partner starter](https://github.com/Zennopay/zennopay-partner-starter)
for a runnable backend.

`getOrCreateCheckoutOperation` must bind a checkout attempt to the authenticated
user, persist one idempotency key, and return the same authorized amount and
attestations on retries. `saveCheckoutIntent` records the intent's ownership
before the session is returned to the app.

```ts
import crypto from "node:crypto";
import express from "express";

const ZENNOPAY_BASE = process.env.ZENNOPAY_BASE_URL!;   // https://api.sandbox.zennopay.com
...
app.post("/wallet/checkout-session", async (req, res) => {
  ...
  const operation = await getOrCreateCheckoutOperation(user, req.body);
  const amountUsdCents = operation.amountUsdCents; // server-authorized wallet amount
  ...
  const body = JSON.stringify({
    ...
    kyc_attestation: {
      verified: true,
      method: "your_kyc_v2",
      verified_at: operation.kycVerifiedAt, // persisted attestation for this attempt
      id_type: "passport",              // which gov ID the user was KYC'd on
      id_country: "IN",
    },
    sanctions_attestation: {
      clean: true,
      screened_at: operation.sanctionsScreenedAt,
    },
  });
  ...
  await saveCheckoutIntent(operation.id, user.id, intent_id);
  res.json({ intentId: intent_id, sessionToken: session_token });
});

// ── Refresh: re-mint the token for the SAME intent ──────────────────────
// Drives the SDK's refreshSession hook. Amount and corridor stay bound to
// the existing intent; current user identity and attestations are sent again.
app.post("/wallet/checkout-session/:intentId/refresh", async (req, res) => {
  const user = await requireAuthedUser(req);
  const { intentId } = req.params;
  await assertUserOwnsIntent(user, intentId); // look up your own record of the intent
...
|---|---|---|
| Scope | One intent | The user (derived from the intent's `partner_user_id`) — **not** intent-bound |
| Lifetime | short (a few min) | TTL 300s |
| Use | Single-use (consumed by confirm) | **Reusable** — polls a pending receipt |
| Power | Authorizes one debit | **Read-only** — only `GET /receipt` |
| Minted by | Zennopay, on intent create | Zennopay, on `POST /v1/payment_intents/{id}/receipt_token` |

Request one on demand when the user taps a history row (you pass that row's
intent id; Zennopay derives the user from the stored intent). The reference
[`zennopay-partner-starter`](https://github.com/Zennopay/zennopay-partner-starter)
(v0.2.0+) ships the route as `POST /receipt-token { intent_id } → { receipt_token,
