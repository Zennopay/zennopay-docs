...
fun PayButton(session: CheckoutSession, viewModel: WalletViewModel) {
    val present = rememberZennopayLauncher(
        ...
        when (result) {
            ...
            is PaymentResult.Pending   -> viewModel.onPaymentPending(result.intentId)
    ...
    private fun startPayment(intentId: String, sessionToken: String) {
        Zennopay.presentCheckout(
            ...
            when (result) {
                ...
                is PaymentResult.Pending   -> showPending(result.intentId)
                is PaymentResult.Canceled  -> Unit      // no money moved
...
callback fires exactly once with the checkout result. The session token is
...
`PaymentResult.Pending` means the user left during processing or polling
timed out. The payment may still settle;
...
- **Sandbox end-to-end:** run `ZennopayConfig.SANDBOX` with a session from your
  sandbox backend; delivery runs on the sandbox rail and the intent reaches
  `captured`.
