...
<Frame caption="One payment, end to end, in the demo wallet app — all rendered by the SDK.">
...
These are real captures from the [demo wallet app](#demo-app) paying a VietQR
merchant on staging.
...
<Steps>
  ...
  <Step title="PaymentSheet: slide to pay → result">
    <Frame caption="Slide-to-pay fires the confirm exactly once; the sheet polls to a terminal state and shows the receipt.">
      <img src="/images/sdk-demo/04-sheet-success.png" width="280" alt="The success screen: Payment sent, with merchant and amount details." />
    </Frame>
  </Step>
...
|---|---|---|---|
| [iOS](/sdks/ios) | `Zennopay` (Swift Package) | `https://github.com/Zennopay/zennopay-ios-sdk` | iOS 16, Swift 5.9 |
| [Android](/sdks/android) | `com.zennopay:sdk` (Maven) | `implementation("com.zennopay:sdk:0.2.0")` | minSdk 24, Compose |
| [Flutter](/sdks/flutter) | `zennopay_flutter` (pub.dev) | `zennopay_flutter: ^0.1.0` | Flutter 3.19, Dart 3.4 |
| [React Native](/sdks/react-native) | `@zennopay/react-native` (npm) | `npm install @zennopay/react-native` | RN 0.70 (bridges the native SDKs) |

Every platform has the same shape: one `present` call in, one `PaymentResult`
out — `completed` / `canceled` / `failed` (Flutter and React Native also
surface a first-class `pending` for poll timeouts).
...
implementation, in [Build your session endpoint](/sdks/session-endpoint).
...
- **Status polling, retries, and recovery** — including re-minting an expired
  ...
  state after a process death mid-confirm.
- **Per-user regulatory limits** — Zennopay enforces corridor limits
  automatically (Vietnam: ₫5,000,000 per transaction, ₫10,000,000 per day,
  ₫25,000,000 per month per user) and the sheet renders the right copy.
...
integration that exercises the [session endpoint contract](/sdks/session-endpoint)
...
<Warning>
  ...
  [iOS](/sdks/ios) and [Android](/sdks/android) pages.
</Warning>
