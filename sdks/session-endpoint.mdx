...
Every [PaymentSheet](/sdks/overview) presentation starts with one round-trip to
...
|---|---|---|
| Publishable key (`pk_test_...` / `pk_live_...`) | In your app — safe to ship | Identifies your integration |
| Secret key (`sk_test_...` / `sk_live_...`) + key ID | Server only — never in a client | HMAC-signing server-to-server calls |
...
<Warning>
  ...
  government ID (passport number, national ID). Zennopay enforces per-user
  regulatory limits against this ID (Vietnam: ₫5,000,000/transaction,
  ₫10,000,000/day, ₫25,000,000/month), so it must be stable per user.
</Warning>
...
const ZENNOPAY_BASE = "https://api.staging.zennopay.com"; // api.zennopay.com in prod
