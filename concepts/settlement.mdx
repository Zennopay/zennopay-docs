...
Zennopay runs daily settlement against both payout providers and a daily
reconciliation pass against the partner FBO. Everything balances by end of
day, or it lands in a break queue for ops review.

## The daily cycle

```text
Throughout the day:
  - Transactions debit Partner FBO; provider success drains the
...
End of business day (UTC):
  ...
  2. GLN top-up:    Zennopay commercial  ──(refill float)──>  GLN pre-fund
  3. 9pay top-up:   Zennopay commercial  ──(refill float)──>  9pay pre-fund
...
### 3. Provider pre-funding match

`receivables.gln.pre_funded` (Zennopay's ledger) must match GLN's reported
pre-funding balance from their daily settlement CSV. Same for 9pay.
...
If a transaction is stuck mid-flow — provider API timed out, no confirmation
either way — Zennopay holds the entry in `clearing.in_flight` and runs a
status-query against the provider's API periodically.
...
|---|---|
| **Daily** | The five reconciliation checks above; clearing accounts zeroed; provider drawdowns top up |
...
- **Daily transaction summary** — per-corridor gross TPV, gross margin,
  provider fees, net margin, recon-break count.
- **Monthly statement** — P&L-style summary for the period.
- **Monthly invoice** — commitment fees due (if any), with a margin-collected
  reference summary.
- **Reconciliation report (per corridor)** — provider CSV vs Zennopay's
  ledger, with any flagged discrepancies.
...
Sub-cent discrepancies from provider FX (sub-$0.01 per transaction) are
