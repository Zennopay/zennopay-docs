...
flowchart LR
    subgraph Partner
        ...
        BE[Partner backend<br/>creates intent HMAC-signed]
    end
    subgraph Zennopay
        API[Zennopay API<br/>mints session token]
    ...
    API -- "session token" --> BE
    App -- "scan / confirm / status<br/>(session token)" --> API
...
1. **One credential.** Your backend signs API calls with a long-lived HMAC
   secret; Zennopay mints the short-lived, single-intent session token your app
   holds. No key pair to generate or register. See
