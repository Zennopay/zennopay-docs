...
description: "Try QR Payments and Platform Payouts endpoints against the sandbox server."
---

Use the interactive playground below to exercise the Zennopay API
...
<Note>
  ...
  **sandbox** HMAC key (`{partner}_sandbox_*`) — production keys are
  rejected by the sandbox host. The alternate `api.sandbox.zennopay.in` host is
...
- For Platform Payouts, an environment-scoped prefund balance and enabled
  payout route. Payout calls are server-to-server and remain HMAC-signed.
...
- **Beneficiaries** — `POST /v1/beneficiaries`
- **Platform Payouts** — `POST /v1/payouts`, `GET /v1/payouts`,
  `GET /v1/payouts/{payout_id}`, and
