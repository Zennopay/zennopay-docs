...
<ParamField path="intent_id" type="string" required>
  ...
  `0198f31a-7c2b-7d91-9fa2-112233445566`. The partner identified by the HMAC key must **own** it.
</ParamField>
...
production. The `.in` hosts remain supported alternates for existing
integrations. See [Environments](/api-reference/environments).

<CodeGroup>
```bash Sandbox
curl -X POST https://api.sandbox.zennopay.com/v1/payment_intents/0198f31a-7c2b-7d91-9fa2-112233445566/receipt_token \
...
```bash Production
curl -X POST https://api.zennopay.com/v1/payment_intents/0198f31a-7c2b-7d91-9fa2-112233445566/receipt_token \
