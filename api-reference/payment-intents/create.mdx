...
<ParamField body="kyc_attestation" type="object">
...
<ParamField body="sanctions_attestation" type="object">
  Your per-payment sanctions attestation, from your real screening system:
  `{ clean, screened_at }`.
</ParamField>

<Note>
  Send both KYC and sanctions attestations to receive a Zennopay-minted session
  token. Deployments explicitly provisioned for partner-minted session JWTs may
  omit both; do not mix one auth model into the other.
</Note>

## Response

<ResponseField name="intent_id" type="string">
  Zennopay intent ID, format `<intent_uuid>`. Use this for subsequent SDK and webhook
  correlation.
</ResponseField>
