...
When a [session token](/authentication#qr-payments-client-session-token) expires mid-flow,
...
<ParamField path="intent_id" type="string" required>
  The intent to re-mint a session token for, e.g. `0198f31a-7c2b-7d91-9fa2-112233445566`. The
  partner identified by the HMAC key must own it. Refreshing a token does not
  reset the intent or start another payment. For a completed payment, use a
  [receipt token](/api-reference/payment-intents/receipt-token) to reopen its
  receipt.
</ParamField>
...
<CodeGroup>
```bash Sandbox
curl -X POST https://api.sandbox.zennopay.com/v1/payment_intents/0198f31a-7c2b-7d91-9fa2-112233445566/session \
...
```bash Production
curl -X POST https://api.zennopay.com/v1/payment_intents/0198f31a-7c2b-7d91-9fa2-112233445566/session \
