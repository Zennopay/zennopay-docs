...
<Note>
  Existing integrations may continue using `api.zennopay.in` and
  `api.sandbox.zennopay.in`. New integrations should use the canonical `.com`
  hosts. See [Environments](/api-reference/environments).
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Re-mint session token" icon="rotate" href="/api-reference/payment-intents/session">
  ...
  <Card title="Mint receipt token" icon="receipt" href="/api-reference/payment-intents/receipt-token">
