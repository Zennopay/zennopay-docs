...
description: "Verify signed payment-intent lifecycle events."
---

This page covers QR Payments events. Platform Payouts has a separate event
contract in [Payout webhooks](/platform-payouts/webhooks).

<Note>
  Webhook signatures share the HMAC canonicalization scheme used for
  server-to-server requests. See [Authentication](/authentication).
</Note>
...
function verifyWebhook(headers, rawBody, secret, requestPath) {
