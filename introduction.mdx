...
<Card title="Get started" icon="bolt" href="/quickstart" horizontal cta="Start building">
  ...
  first HMAC call, mint a session token, present the PaymentSheet.
</Card>
...
<CardGroup cols={3}>
  ...
  <Card title="Session endpoint" icon="server" href="/payments/session-endpoint">
    The one backend route every payment starts with: create an intent, mint the
    session token.
  </Card>
...
3. Partner backend mints a short-lived **session token** identifying that
   specific user + intent.
4. Partner app opens the Zennopay SDK with the `intent_id` + token.
...
- **Two-layer auth.** Long-lived HMAC on the server side, short-lived
  per-session token on the client side — the dual-token pattern common to modern
  payment APIs.
