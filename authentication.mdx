...
| Product | Server-to-server calls | Client credential |
|---|---|---|
| QR Payments | HMAC for intent creation and partner reads | Session token for PaymentSheet; receipt token for receipt reads |
| Platform Payouts | HMAC for beneficiaries and payouts | None; the product has no client-side API |

The HMAC contract below is shared. Session tokens, receipt tokens, and app
package allowlisting apply only to QR Payments.
...
live mode, `https://api.sandbox.zennopay.com/v1/*` in sandbox — see
...
{X-Zennopay-Nonce}\n
{SHA256_HEX of non-empty request body, or empty string when no body}\n
```
...
For partner API calls, `REQUEST_PATH` is the URL pathname, such as
`/v1/payouts`; exclude the host and query string. Use an uppercase HTTP method.
For outbound webhook verification, use the endpoint pathname **including its
query string**, if configured. See [Webhooks](/api-reference/webhooks).

The last line is the lowercased SHA256 hex of the exact bytes of a non-empty
request body. If there is no body, use an empty string for this line, including
for a bodyless POST. Sign this string with HMAC-SHA256 using your signing secret, then
...
4. **Signature:** reconstruct the canonical request, recompute HMAC, compare
   in constant time.
5. **Nonce uniqueness:** not seen in the replay window.
...
## QR Payments client session token
...
## QR Payments app package allowlist

The session token authorizes requests for a user and intent. The app package
allowlist checks the `X-Zennopay-Package` identifier sent by the SDK against
your registered app identifiers. This header check is not device or app
attestation; continue to protect session tokens as bearer credentials.
...
Authentication defines credentials and signing only. Follow the product guide
for request order, resource states, and retry behavior:

- [QR Payments quickstart](/quickstart)
- [Build the PaymentSheet session endpoint](/payments/session-endpoint)
