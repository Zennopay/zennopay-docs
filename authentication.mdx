...
Zennopay uses two distinct auth layers in parallel — the dual-token pattern
common to modern payment APIs: a long-lived shared secret on the server side,
a short-lived per-session token on the client side.
...
mobile app → the Zennopay SDK → `checkout.zennopay.in`.
...
```
https://checkout.zennopay.in/flow/zp_AbCd1234/scan#token={jwt}
```
...
Partner backend creates intent:
    POST https://api.zennopay.in/v1/payment_intents  (HMAC-signed)
...
Partner app calls SDK:
    ...
    |
    v
SDK opens https://checkout.zennopay.in/flow/zp_AbCd1234/scan#token=eyJ...
    |
    v
Checkout web:
