---
title: "QR Payments quickstart"
description: "Create a sandbox payment intent and present PaymentSheet."
sidebarTitle: "Make a QR payment"
---
...
<Note>
  Arrange sandbox access with **partners@zennopay.com**. You need a sandbox
  key ID and signing secret, a registered app identifier, an enabled Vietnam
  corridor, and a configured wallet and prefund balance. This guide uses
  `vn_vietqr`; see [Corridors](/concepts/corridors) for availability.
</Note>
...
<Info>
  **Environments.** Point everything at the sandbox host
  ...
  The `.in` hosts remain supported alternates for existing integrations. New
  integrations should use the canonical `.com` hosts above.
</Info>
...
if (!res.ok) throw new Error(`Zennopay request failed: ${res.status}`);
const { intent_id, session_token, session_expires_at } = await res.json();
```
</CodeGroup>

Generate a fresh timestamp, nonce, and signature for every request; the cURL
values above only illustrate the request format. Persist the idempotency key
with your checkout operation and reuse it on retries.
...
Zennopay.presentCheckout(
  from: viewController,
  intentID: "0198f31a-7c2b-7d91-9fa2-112233445566",
  sessionJWT: "zpst_9b1f..."
) { result in
  // .completed / .canceled / .failed / .pending
}
```
...
The API returns `session_token`. Pass that value unchanged to `sessionJWT`
on iOS or `sessionJwt` on Android, Flutter, and React Native. These SDK
parameter names do not require you to mint or sign a token.
